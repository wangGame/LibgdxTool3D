...
public class OutlineShaderProvider extends PBRDepthShaderProvider
{
	...
	private static Config createConfig(int numBones) {
		...
		config.vertexShader = Gdx.files.internal("gltf-outline-ext.vs.glsl").readString();
		config.fragmentShader = Gdx.files.internal("gltf-outline-ext.fs.glsl").readString();
		return config;
	}
