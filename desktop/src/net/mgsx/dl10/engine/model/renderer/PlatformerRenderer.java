...
public class PlatformerRenderer {
	...
	public PlatformerRenderer(PlatformerEngine engine) {
		...
		sceneManager = new SceneManager(createColorShader(maxBones),
				PBRShaderProvider.createDefaultDepth(maxBones), new SceneRenderableSorter());
	...
	private static ShaderProvider createColorShader(int maxBones){
		...
		config.vertexShader = Gdx.files.internal("gltf-ceil-shading.vs.glsl").readString();
		config.fragmentShader = Gdx.files.internal("gltf-ceil-shading.fs.glsl").readString();
	...
	public void update(float delta){
		...
		renderableProviders.addAll(engine.level.scenes);
...
//					sceneManager.getScenes().removeValue(e.model.scene, true);
					Array<RenderableProvider> renderableProviders1 = sceneManager.getRenderableProviders();
					renderableProviders1.removeValue(e.model.scene, true);
		...
		Array<RenderableProvider> renderableProviders2 = sceneManager.getRenderableProviders();
		for (RenderableProvider renderableProvider : renderableProviders2) {
			if (renderableProvider instanceof Scene) {

				((Scene) renderableProvider).modelInstance.calculateTransforms(); // XXX because no master anims
			}
		}
	}
	...
	public void render(Viewport viewport) {
		...
		sceneManager.render();
		if(true){
			OutlineShader.extrusionRate = .05f * .6f;
