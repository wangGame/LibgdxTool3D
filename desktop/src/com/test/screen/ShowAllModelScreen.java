...
public class ShowAllModelScreen extends BaseScreen3D {
    ...
    public void initView() {
        ...
        ModelActor3D bg = new ModelActor3D(model);
        stage3D.addActor(bg);
        ...
        ModelActor3D abutton = new ModelActor3D(Asset3D.getAsset3D().getModel("grandpa/a-button.g3db"));
    ...
    private void commonMathod(BaseActor3DGroup group,String modelPath,String texture, float i,float y) {
        BoundingBox bounds;
        float max;
        ModelActor3D adbuddiz = new ModelActor3D(Asset3D.getAsset3D().getModel(modelPath));
